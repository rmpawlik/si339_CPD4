...
def split_after_two_sentences(html_string):
    # Remove the <p> tags temporarily for easier processing
    clean_string = re.sub(r'<\/?p>', '', html_string)
    
    # Use regex to find sentence-ending punctuation (., !, or ?) followed by a space
    sentences = re.split(r'(?<=[.!?])\s+', clean_string)
    
    # Join the first two sentences back together
    first_part = ' '.join(sentences[:2]).strip()
    
    # The remaining part after the first two sentences
    remaining_part = ' '.join(sentences[2:]).strip()
    
    # Re-add the <p> tags
    first_part_html = f'<p>{first_part}</p>'
    remaining_part_html = f'<p>{remaining_part}</p>' if remaining_part else ''
...
def csv_to_html(csv_filename, output_folder):
    ...
    with open(csv_filename, mode='r', newline='', encoding='utf-8') as csvfile:
        ...
        h2_text = rows[1][0]
        link_url = rows[2][0]
        summary_text_temp = rows[3][0]
        summary_button_span = '<span id="hidden-summary" style="display:none;">' 
        summary_text, remaining_summary = split_after_two_sentences(summary_text_temp)
        summary_text = f"{summary_text} {summary_button_span}{remaining_summary} </span>" if remaining_summary else summary_text
...
<script src="https://kit.fontawesome.com/97514ed53b.js" crossorigin="anonymous"></script>
   ...
   <main id = "main">
    ...
    <section class="summary" id = "summary">
      <h2 class="summary_header">Race Summary
       <button id="toggle-summary-btn" onclick="toggleSummary()">Show More</button></h2>
      {summary_text}
    </section>
...
"""
    ...
    if (hiddenText.style.display === "none") {
        ...
        for row in rows[4:]:
            ...
            if len(row) == 3:
                if row[0] == "Place":
                    html_content += f"<tr><th tabindex=0>{row[0]}</th><th tabindex=0>{row[1]}</th><th tabindex=0>{row[2]}</th></tr>\n"

                else:
                    html_content += f"<tr><td tabindex=0>{row[0]}</td><td tabindex=0>{row[1]}</td><td tabindex=0> {row[2]}</td></tr>\n"
